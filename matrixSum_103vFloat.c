...
// -------------------------------------------------------------------------------
void SumMatv1f(int n, float* A) { //JI
    int i, j, k;
    float sum=0.0;
...
void SumMatv2f(int n, float* A) { //IJ
    int i, j, k;
    float sum=0.0;
    /* This is ijk loop order. */
    for (i = 0; i < n; i++)
...
//printf( " input n = %d " , n);
  ...
  for(int s=0;s<samples;s++){
    // Start measuring time
            clock_t start = clock();
            SumMatv4f(n, A);     //select multMatv1, multMatv2, multMatv3, multMatv4
    // Stop measuring time and calculate the elapsed time
           ...
           printf( "%s\t   %05d  %2.4f   %2.4f \n" , "ver4f_local ", n, seconds, timeNormalized);
